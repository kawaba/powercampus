 ...
 *
*/
public class Kougi extends SuperPlayer implements BbsVar{
	 ...
	 * _contentPlan は、現状は、そのまま kougi.html に差し込まれている（509行目）
...
*/
	...
	public void display(boolean editmode){
		...
		if(ksd.size()==0){
			...
			if(LOG.fa) LOG.println("■ seqnumber = " + getDisplaySeqnumber());
			// 自動設定を要求する "-1" であれば、表示すべきレコードのシーケンス番号を検索してhtbにセットする
			if(isEmpty(seq)||seq.equals("-1")){
				String	sqStr	=	getDisplaySeqnumber(); // ここでコール
				if(sqStr.equals("-1")){
	...
	void	sect_list(Vector exHtml){
		...
		for(int i=0; i<n; i++){
			...
			/****************************************************************
			  旧 putParameter("_sect_content"	,(ksdrec.content()).trim() );			// セクション内容
			 *
			 *
			 * 新
			 * 講義内容の説明をPMLで記述するので、表示用のHTMLに変換する
			...
			putParameter("_sect_content" ,HTML_text);
