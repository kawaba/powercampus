...
public class Tregularizer extends Object{
	...
	public String	toGeneralSet(String	str){
		...
		//String	str0	= Cp932.toJIS(str);
		//String	str1	= substitute(str.trim(),"　"," ");
    ...
    public	String toDigit(String s){
        ...
        for(int i=0; i<len; i++){
            char ch = matchDigit(s.charAt(i));
			if(ch == '*'){
				if(LOG.fa) LOG.println("-- value :" + bf.toString() + "(" + (bf.toString()).length() + ")");
    ...
    public	String unitString(String str){
        ...
        for(int i=len-1; i>=0; i--){			// 右端から見ていく
			...
			if(ch != '*'){						// 数字の時
				if(bf.length()==0){
					if(LOG.fa) LOG.println("-- unit :" + bf.toString() + "(" + (bf.toString()).length() + ")");
					return "";	// 長さゼロなら""を返す
				}
				bf.reverse();
				if(LOG.fa) LOG.println("-- unit :" + bf.toString() + "(" + (bf.toString()).length() + ")");
