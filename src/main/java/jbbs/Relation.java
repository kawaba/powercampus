...
/**
 * オーナーIDでフォーラムを区別する
 * オーナーが所有するフォーラムのリストにたいして
 * 削除あるいはリレーションキーの付け替えを行う
 * 
 * （オーナーID はBBSライブラリを呼び出す側で設定されている）
 * 
	#
	# ############
	#	Relation
	# ############
	#
	<program $jbbs.Relation>
		<dispatch  html=Relation.html   number=3140  class=jbbs.Relation />
		<variable>
		  <receive  NUMBER   STAMP  GROUP  BBS_OWNER_KEY  UID MAIL UNAME  HOMEURL BBS_RELATION DIVISION />
 ...
 * 1. receive 
 * 2. accept
 * 		forumkey	--	削除または関係付けするために選択されたフォーラムのフォーラムキー
 * 3. keep
 * 		cnt			--	（リレーションの一致しない）レコードの総数
 * 4. form 
 *		relate<n>	--	関係付けるフォーラムのフォーラムキー
 * 
 *   
 ...
 *     
 */
public class Relation extends SuperPlayer implements BbsVar{
	BbsConf				conf;
	//
	BbsForumDB			db;
	String				ownerid;
	...
	public	Relation(){
		super();
		if(LOG.fa)	LOG.println("■ Relation #コンストラクタ");
	}
	...
	public	void initialize(PrintWriter out, Hashtable htb, Param para){
		...
		//
		if(LOG.fa)	LOG.println("■ Relation #initialize()");
		conf			=	createBbsConf(para.get("BBS_CONF_PATH"),htb);		
		/*
		 * ownerkey != userid という前提
		 * 学生やTEの場合は同じではない 
		 */
		ownerid			=	strHash(htb,BBS_OWNER_KEY);	// ownerkey(==teUid)
		/*
		 * リレーションキー
		 */
	...
	public BbsConf	createBbsConf(String BbsConfPath, Hashtable ht){
		if(LOG.fa)	LOG.println("■ Relation #createBbsConf()");
		/* 
		 * BBSコンフィギュレーションクラス
		 */
	...
	public	String	dispatch(){
		if(LOG.fa) LOG.outHash(htb,"■ Relation #dispatch()");
		...
		if(cmd.equals("VIEW")){
			 ...
			 * スレッドやポストも再帰的に削除する
			 */
			if(permission( getParameter(BbsForumDB.FORUM_KEY) )){
				delete(getParameter(BbsForumDB.FORUM_KEY));
				disp_mode	=	DISP_NEW;
	 ...
	 */
	void	chgRelation(){
	    String ns	=	getParameter("cnt");
        if(LOG.fa){
            LOG.println("■Relation #chgRelation()");
            LOG.println("    ns = " + ns);
        }	    
	    int		n	=	Integer.parseInt(ns);
	    for(int i=0; i<n; i++){
	        /*
	         * related<n> は選択用チェックボックスの名前で，値は選択されたフォーラムのフォーラムキー．
	         * ここでは全てのチェックボックスを調べて，チェックが入っていればそのフォーラムのリレーションキーを
	         * このフォーラムのものに付け替える処理を行う
	         */
	        String	selectedFormKey	=	getParameter("related" + String.valueOf(i));			
	        
	        if(!isEmpty(selectedFormKey)){// チェックされているか
	            if(LOG.fa){
	                LOG.println("    relatedKey-" + String.valueOf(i) +"=" + selectedFormKey);
	            }
	            changeRelationkey(selectedFormKey);
	            
	        }
	    }
	}
	/**
	 * リレーションキーをこのクラスの現在値に書き換える
	 * 
	 * @param relatedKey	対象とするフォーラムのリレーションキー
	 * 
	 */
	void	changeRelationkey(String selectedFormKey){
	    
	    db.updateRelation(ownerid, selectedFormKey, relation);
	...
	String	permissionCreate(){
		...
		if(user==null)	return	"★ ユーザー情報を登録してください";
	...
	Hashtable	createInfoRecord(String forumOwnerKey){
		if(LOG.fa) LOG.println("■ Relation #createInfoRecord()");
	...
	boolean	permission(String forumkey){
		if(LOG.fa) LOG.println("■ Relation #permission()");
		/*
		 * アクセスしているユーザーIDフォーラム所有者IDと同じならば、更新・削除権がある
		 */
	...
	public  void	display(boolean editmode){
		if(LOG.fa)	LOG.println("■ Relation #display(boolean editmode)");
		if(!editmode){
			htb.put(MESSAGE,"");
		}
		/*
		 * 表示総数をシステムハッシュに記憶するため
		 * ウェブ変数の置き換えは最後に行う
		 */
		clearPresetFlag();
	...
	// 内容は、key で特定される．
	//
	public void	forumList(Vector exHtml){

		Vector	v	=	new Vector(15);
		/*
		 * リレーションキーが一致するものは表示しない
		 * isNolist() のためにフォーラムの数を残す
		 */			
		forumCounts	=	db.readNotRelatedBbsForums(ownerid, relation, v);
		/*
		 * 総数をシステムハッシュに記憶する
		 */
		int	n	=	v.size();
		putParameter("cnt", String.valueOf(n));

		for(int i=0; i<n; i++){
			/*
			 * 順序番号
			 */
		    putParameter("k", String.valueOf(i));
	...
	void	setThreadInfo(Hashtable h, Vector v, String forumkey, String handleFlag){
		if(LOG.fa)	LOG.println("■ Relation #setThreadInfo()");
		int		threads			=	0;
		int		posts			=	0;
	...
	Vector	getPostRecords(String forumkey, String threadkey){
		if(LOG.fa)	LOG.println("■ Relation #getPostRecords()");
		/*
	...
	String	getUserFaceIconURL(String forumOwnerKey, Hashtable inforec){
		if(LOG.fa) LOG.println("■ Relation #getUserFaceIcon()");
	...
	int	getSysFaceIconNumber(){
		if(LOG.fa) LOG.println("■ Relation #getSysFaceIcon()");
		/*
		 * システムアイコンファイル一覧を得る
		 */
	...
	String	userIconUrl( String forumOwnerKey, String fname){
		if(LOG.fa) LOG.println("■ Relation #getFaceIcon()");
	...
	public	String	getContent(String content, String forumkey){
		 ...
		 */
		//if(DBG.fa) DBG.println("■ Relation #getContent : fileName = " + conf.pmlTemplatePath());
	...
	public	String	getAttachmentHTML(Hashtable record){
		if(LOG.fa)	LOG.println("■ Relation #getAttachmentHTML()");
	...
	String	getFilename(Csv cs, int pos){
		if(LOG.fa)	LOG.println("■ Relation #getFilename()");
	...
	String	getColorClass(Csv cs, int pos){
		if(LOG.fa)	LOG.println("■ Relation #getColorClass()");
	...
	boolean	isAttachment(Csv cs, int pos){
		if(LOG.fa)	LOG.println("■ Relation #isAttachment()");
