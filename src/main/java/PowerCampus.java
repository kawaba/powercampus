...
public class PowerCampus extends HttpServlet  implements PCvar {
	...
	public void init() throws ServletException {
		...
		// ライセンスキーファイル
		keyFilePath		= confpath  + "license.key";		
		...
		//
		// ライセンスキー認証クラス(ここで作成しておく)
       	walker	=	null;
		if(isExistLicenseKey(keyFilePath)){
       		walker	= new	Moonwalker(keyFilePath,sysLog);
       	}
		/*
		 * ログファイルを開く
		 * 
		...
		if(property.get("LOG").equals("YES")){
			...
			LOG.logOn(sysLog);
			LOG.println("log is opened.");
			
			epml.tools.DBG.logOn(accessLog);
			epml.tools.DBG.println("epml-log opened.");
		}
		/*
		 * その他の初期化（オーバーロード用）
		 */
    ...
    void setSystemPath(){
		 ...
		 */
		pcpath		=	getServletContext().getRealPath(getInitParameter("baseUri"));  //  "/" が定義されている
		confpath	=	pcpath	+	getInitParameter("confdir") + FS; // "system\config" が定義されている
		pcConfPath	=	confpath	+	"pc.conf";  //  "/system\config\pc.conf" になる
		
        // システムコンフィギュレーションファイル全体
        sysConfStr	=	Gear.getFileData(pcConfPath);	// pc.confを読み込んだ文字列データ（Shift_Jis）
        
	    //  システムプロパティオブジェクトを得る
        property	= 	getConfPoperty(sysConfStr);		// pc.confからプロパティオブジェクトを作る
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        if(LOG.fa){
            ...
            LOG.println("OS="+os);
            LOG.println("P=" + Boolean.getBoolean(p));
        } 
	...
	SuperPlayer	getCtrlObj(String className, Hashtable htb ,PrintWriter out){
		...
		SuperPlayer	ctrl		=	null;
  		try{
  			...
  			ctrl = (SuperPlayer)(name.getDeclaredConstructor().newInstance());
     ...
     public	void	relay(String fromKey, SuperPlayer from, String gotoKey, Dispatch dispatch, PrintWriter out, Hashtable htb, 
		...
		if(gotoKey.equals( DISPATCH_RETURN )){
			...
			try{
				...
				// ログイン画面に遷移するよう変更した 2022.5
				gotoKey = getLoginProcessName();
			}
			
			// nullでないときだけhtbに代入するよう変更 2021.05
			if(newHash!=null) {
				htb	=	newHash;
			}
    ...
    public void selfReturn(String fromKey, SuperPlayer from, Dispatch dispatch, PrintWriter out, Hashtable htb, Param conf){
     ...
     public void	initialDisplay(Dispatch dispatch, String gotoKey, PrintWriter out, Hashtable htb, Param conf){
		...
		if(LOG.fa)LOG.println("★className =" + className);
				
		if(isEmpty(className)){
		
			if(LOG.fa) LOG.println("★className は空");
			/*
	    	 * エラーメッセージ画面を表示する
	    	 */
		...
		try{
			Class<?> 	name	=	Class.forName(className);
			if(LOG.fa)LOG.println("★ 取り出したクラス名=" + className);

			
			sp = (SuperPlayer)(name.getConstructor().newInstance());
			
			//sp = (SuperPlayer)(name.getDeclaredConstructor().newInstance());
			
		}catch(Exception e){
			
			if(LOG.fa)LOG.println("★ クラス名が見つからない");
			if(LOG.fa)LOG.println(e.toString());
     ...
     * @return
     */
    boolean	isExistLicenseKey(String keyFilePath){
		if(LOG.fa) LOG.println("■PowerCampus #isExistLicenseKey()");

    	File fp	=	new File(keyFilePath);
    	return	fp.exists();
    }
    ...
    public	void	initialize(PrintWriter out, Hashtable  htb, Param para, HttpServletResponse response) throws IOException{
        ...
        if(myBroker==null) {
        	if(LOG.fa) LOG.println("★myBroker -- null ");
		 ...
		 * 分類区分を教師に設定しておく
		 */
		htb.put(DIVISION, DIV_TEACHER);	// 区分は教師
		/*
		 * 暗号化用変数の準備
		 */
        for(int i=0; i<10; i++){
		...
		/*
		 * 教師ID発生用のナンバーファイルへの絶対パス
		 */
		htb.put(NUMBER_FILE, numberFile);
     ...
     * @param request
     */
    void 	license(PrintWriter out, Hashtable htb, Param para, HttpServletRequest request){
		if(LOG.fa) LOG.println("■PowerCampus #license()");

    	SuperPrint	sp	=	new SuperPrint(out);

    	if(walker==null){
			htb.put("_msg","★ ライセンスキーが見つかりません．");
          	sp.repPrint(para.get("SYSTEM_ERROR"),htb);
			out.close();
			return;
    	}
    	
		keyUser			= 	para.getSysadmin();
		if( isEmpty(keyUser) ){
			htb.put("_msg","★ システム設置責任者(sysadmin)のメールアドレスを設定してください");
            sp.repPrint(para.get("SYSTEM_ERROR"),htb);
			out.close();
			return;
		}
		if(!walker.checkStar(para.getViewSwitch(), 
							 para.getServer(), 
							 para.getSystemAdmin(), 
							 para.getSystemAdminMail(), 
							 para.getMailhost())){
			
			htb.put("_msg","★ ライセンスを確認できませんでした．");
          	sp.repPrint(para.get("SYSTEM_ERROR"),htb);
			out.close();
			return;
		}
	...
	/**
	 * システムかどうかを sysadmim キーワードを見て調べる
	 */
	public boolean		isSystem(){
		if(LOG.fa) LOG.println("■PowerCampus #isSystem()");
		//
		if(keyUser.equals(Param.SYSTEM_ADMIN_ID))	return 	true; // 本家システムかどうか
		return	false;
	}
	/**
	 * セッションが有効かチェックする
	...
	public boolean checkSsession(HttpServletRequest request) {
     ...
     void	setBasicVar(Hashtable htb, HttpServletRequest request){
        ...
        HttpSession session = request.getSession(true);            	// セッションオブジェクトの取得
      	...
      	/*   
      	 *   db.MembersInfo()によってmembers テーブルから以下の情報を htb（システムハッシュ）に設定する
      	 * 
      	 * 		
		 ...
		 *  
		 *	戻り先URL(user_url_t)				_url_t, HOMEURL
		 *
      	 * 
