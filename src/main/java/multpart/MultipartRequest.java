...
public class MultipartRequest {
  ...
  public MultipartRequest(HttpServletRequest request,
    ...
    /* マルチパートをパースして送信されたファイルを保存ディレクトリに書き込む
     * 複数ファイルに対応している．また、パラメータもとりだし保存する．
     */
    ...
    while ((part = parser.readNextPart()) != null) { 
      ...
      if (part.isParam()) {
        ...
        String value = paramPart.getStringValue();	// ParamPartクラスを日本語対応に修正している 2003.2.11
        ...
        String fileName = filePart.getFileName();// parser.readNextPart() で取得ファイル名を日本語対応に変更した 2003.3.25
        if (fileName != null) {
          /* if(DBG._tr090) DBG.println("class MultipartRequest#コンストラクタ: 取得したファイル名 = " + fileName );
		   * The part actually contained a file
           * 実際に保存ディレクトリにファイルを書き出す
          **/
          filePart.writeTo(dir);
		  
		  /*
		   * ファイル名をキーにして、ハッシュテーブル files にUploadedFileオブジェクト
		   * （ディレクトリ名、ファイル名、タイプ）を記録する 
		   */
