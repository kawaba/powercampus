  ...
  public Part readNextPart() throws IOException {
    ...
    while (enm.hasMoreElements()) {
      ...
      if (headerline.toLowerCase().startsWith("content-disposition:")) {
		...
		// 日本語対応にするための変更
		// ---- 2003.3.25 ここから -----------
        String _name = dispInfo[1];
        String _filename = dispInfo[2];
        String _origname = dispInfo[3];
		//
		if((_name != null)&&(_name.length()>0)){
			name 	 = new String(_name.getBytes("8859_1"),"JISAutoDetect");
		}else{
			name	= _name;
		}
		//
		if((_filename != null)&&(_filename.length()>0)){
			filename 	 = new String(_filename.getBytes("8859_1"),"JISAutoDetect");
		}else{
			filename	= _filename;
		}
		//
		if((_origname != null)&&(_origname.length()>0)){
			origname 	 = new String(_origname.getBytes("8859_1"),"JISAutoDetect");
		}else{
			origname	= _origname;
		}
		// ---- ここまで ---------------------
